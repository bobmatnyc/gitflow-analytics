...
help-release: ## Show version and release targets
	...
	@echo "  $(CYAN)commit-version$(NC)       Commit version bump and push to main"
	@echo ""
	@echo "$(GREEN)Build:$(NC)"
	@echo "  $(CYAN)build$(NC)                Build package with uv build"
	...
	@echo "  $(CYAN)brew-update$(NC)          Update Homebrew tap formula + push"
	@echo "  $(CYAN)publish-brew$(NC)         Alias for brew-update"
	@echo "  $(CYAN)publish-test$(NC)         Publish to TestPyPI for testing"
	@echo ""
	@echo "$(GREEN)One-Command Workflows:$(NC)"
	@echo "  $(CYAN)publish-all$(NC)          build -> PyPI -> GitHub release -> Homebrew"
	@echo "  $(CYAN)release-all$(NC)          patch + commit-version + publish-all"
	@echo ""
	@echo "$(GREEN)Release Prep Shortcuts (with quality gate):$(NC)"
	@echo "  $(CYAN)release-patch$(NC)        Check + bump patch + quality build"
	@echo "  $(CYAN)release-minor$(NC)        Check + bump minor + quality build"
	@echo "  $(CYAN)release-major$(NC)        Check + bump major + quality build"
	@echo "  $(CYAN)release-verify$(NC)       Show verification links after publish"
	@echo "  $(CYAN)release-dry-run$(NC)      Preview what a release would do"
	@echo ""
	@echo "$(GREEN)CI-Friendly Usage:$(NC)"
	@echo "  make publish-all CONFIRM=yes   Skip all interactive prompts"
	@echo "  make release-all CONFIRM=yes   Full automated release pipeline"
