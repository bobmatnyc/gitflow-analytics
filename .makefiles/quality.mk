# ============================================================================
# quality.mk - Code Quality Targets for GitFlow Analytics
# ============================================================================
...
# ============================================================================
.PHONY: lint lint-ruff lint-fix lint-mypy quality quality-ci pre-publish
.PHONY: format type-check security-check quality-gate
...
lint-ruff: ## Run Ruff linter and formatter check
	...
	@if command -v ruff &> /dev/null; then \
		ruff check $(SRC_DIR)/ $(TEST_DIR)/ $(RUFF_ARGS) || exit 1; \
		echo "$(GREEN)✓ Ruff linting passed$(NC)"; \
		echo "$(YELLOW)🔍 Checking code formatting...$(NC)"; \
		ruff format --check $(SRC_DIR)/ $(TEST_DIR)/ || exit 1; \
...
lint: lint-ruff ## Run linting checks (Ruff-first, maintains compatibility)

type-check: lint-mypy ## Run type checking with mypy (alias for compatibility)
...
lint-fix: ## Auto-fix linting issues (ruff format + ruff check --fix)
	...
	@if command -v ruff &> /dev/null; then \
		...
		ruff check $(SRC_DIR)/ $(TEST_DIR)/ --fix || true; \
...
format: lint-fix ## Auto-format code (alias for lint-fix, maintains compatibility)
...
security-check: ## Run security checks with bandit
	@echo "$(YELLOW)🔒 Running security checks with bandit...$(NC)"
	@if command -v bandit &> /dev/null; then \
		bandit -r $(SRC_DIR)/ -f json -o bandit-report.json || true; \
		echo "$(GREEN)✓ Security check complete (report: bandit-report.json)$(NC)"; \
	else \
		echo "$(YELLOW)⚠ bandit not found. Install with: pip install bandit[toml]$(NC)"; \
	fi
...
quality-gate: ## Legacy compatibility target (runs quality + security)
	@echo "$(BLUE)════════════════════════════════════════$(NC)"
...
quality-ci: ## Quality checks for CI/CD (strict, fail fast)
	...
	ruff check $(SRC_DIR)/ $(TEST_DIR)/ --no-fix; \
	echo "$(YELLOW)🔍 Type checking...$(NC)"; \
	mypy $(SRC_DIR)/ --ignore-missing-imports; \
	echo "$(YELLOW)🧪 Running tests (parallel)...$(NC)"; \
	$(PYTHON) -m pytest $(TEST_DIR)/ $(PYTEST_ARGS) --tb=short
	@echo ""
	@echo "$(GREEN)════════════════════════════════════════$(NC)"
...
clean-test-artifacts: ## Remove test artifacts (HTML, JSON reports in root)
	...
	@rm -f dashboard_test.html report_qa_test.html coverage.json bandit-report.json 2>/dev/null || true
	@rm -rf htmlcov/ .coverage .pytest_cache/ .mypy_cache/ .ruff_cache/ 2>/dev/null || true
...
pre-publish: clean-pre-publish ## Comprehensive pre-release quality gate
	...
	@if command -v pytest >/dev/null 2>&1; then \
		$(PYTHON) -m pytest $(TEST_DIR)/ $(PYTEST_ARGS) || exit 1; \
	else \
