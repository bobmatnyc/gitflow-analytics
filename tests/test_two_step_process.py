...
def test_database_models():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            retrieved = session.query(DailyCommitBatch).filter(
                DailyCommitBatch.project_key == "TEST"
            ).first()
...
def test_metrics_storage():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        records_created = storage.store_daily_metrics(test_date, sample_commits, developer_identities)
