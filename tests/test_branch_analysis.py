...
def compare_strategies(repo_path: str):
    ...
    print(f"Total branches: {len(list(repo.heads))} local, {len(list(repo.remotes.origin.refs))} remote")
    print(f"{'#'*80}")
    ...
    for strategy in ['main_only', 'smart', 'all']:
        count, branches = test_branch_strategy(repo_path, strategy, weeks=52)  # Use 52 weeks for more data
        results[strategy] = {
            'total': count,
