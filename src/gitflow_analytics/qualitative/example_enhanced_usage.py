...
def create_sample_project_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        'story_point_analysis': {
            'total_story_points': 195,
            'average_per_commit': 4.2
        }
    }
...
def demonstrate_enhanced_analysis():
    ...
    for project_key, project_data in list(project_analysis.items())[:3]:  # Show first 3 projects
        ...
        print(f"  Momentum: {momentum.get('classification', 'Unknown')} ({momentum.get('confidence', 0):.2f} confidence)")
    ...
    for dev_id, dev_data in list(developer_analysis.items())[:3]:  # Show first 3 developers
        ...
        print(f"  Pattern: {contribution.get('pattern', 'Unknown')} ({contribution.get('confidence', 0):.2f} confidence)")
        print(f"  Collaboration: {collaboration.get('level', 'Unknown')} ({collaboration.get('score', 0):.1f}/100)")
    ...
    for insight in cross_insights:
        print(f"  • {insight.get('title', 'Unknown')} ({insight.get('priority', 'unknown')} priority)")
    print()
