...
class EnhancedQualitativeAnalyzer:
    ...
    def _prepare_analysis_context(
        ...
        qualitative_by_hash = {}
        if qualitative_data:
            qualitative_by_hash = {q.hash: q for q in qualitative_data}
    ...
    def _analyze_developers(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        for dev_id, dev_commits in commits_by_developer.items():
            ...
            career_recommendations = self._generate_career_recommendations(
                contribution_pattern, collaboration_score, expertise_domains, 
                growth_trajectory, burnout_indicators
            )
            ...
            developers_analysis[dev_id] = {
                ...
                "developer_narrative": self._generate_developer_narrative(
                    ...
                    contribution_pattern, expertise_domains, growth_trajectory
                )
            }
    ...
    def _assess_team_health(self, context: dict[str, Any]) -> tuple[str, float]:
        ...
        if weekly_commits:
            consistency = 100 - (statistics.pstdev(weekly_commits) / max(statistics.mean(weekly_commits), 1) * 100)
            health_factors.append(max(0, min(100, consistency)))
        
        # Developer engagement factor
        if developer_stats:
            active_developers = sum(1 for dev in developer_stats if dev.get('total_commits', 0) > self.thresholds['low_productivity_commits'])
    ...
    def _identify_key_achievements(self, context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        total_commits = len(commits)
        if total_commits > self.thresholds['high_productivity_commits'] * context['weeks_analyzed'] / 12:
            achievements.append({
    ...
    def _identify_major_concerns(self, context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if developer_stats and len(developer_stats) > 1:
            ...
            if gini_coefficient > self.thresholds['bus_factor_threshold']:
                ...
                top_percentage = (top_contributor.get('total_commits', 0) / sum(commit_counts)) * 100
        ...
        velocity_trends = self._analyze_velocity_trends(context)
        if velocity_trends['trend_direction'] == 'declining' and velocity_trends['trend_percentage'] < -20:
            concerns.append({
                "category": "productivity",
        ...
        if developer_stats:
            inactive_devs = sum(1 for dev in developer_stats 
    ...
    def _assess_risk_indicators(self, context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        large_commits = sum(1 for c in commits 
                           if (c.get('filtered_insertions', c.get('insertions', 0)) + 
                               c.get('filtered_deletions', c.get('deletions', 0))) > self.thresholds['large_commit_lines'])
    ...
    def _generate_executive_narrative(
        ...
        narrative_parts.append(f"The development team is currently {health_descriptions.get(health_assessment, 'in an unclear state')}.")
        
        # Velocity trends
        if velocity_trends['trend_direction'] == 'improving':
            narrative_parts.append(f"Team velocity is trending upward with a {velocity_trends['trend_percentage']:.1f}% improvement, averaging {velocity_trends['weekly_average']} commits per week.")
        elif velocity_trends['trend_direction'] == 'declining':
            narrative_parts.append(f"Team velocity shows concerning decline of {abs(velocity_trends['trend_percentage']):.1f}%, requiring immediate attention to restore productivity.")
        else:
            narrative_parts.append(f"Team velocity remains stable at {velocity_trends['weekly_average']} commits per week, providing consistent delivery rhythm.")
        
        # Key achievements
        if achievements:
            high_impact_achievements = [a for a in achievements if a.get('impact') == 'high']
            if high_impact_achievements:
                narrative_parts.append(f"Notable achievements include {', '.join([a['title'].lower() for a in high_impact_achievements[:2]])}.")
        
        # Major concerns
        critical_concerns = [c for c in concerns if c.get('severity') == 'high']
        if critical_concerns:
            narrative_parts.append(f"Critical attention needed for {critical_concerns[0]['title'].lower()} and other high-priority issues.")
        elif concerns:
            narrative_parts.append(f"Some areas require monitoring, particularly {concerns[0]['category']} aspects.")
    ...
    def _classify_project_momentum(self, project_commits: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        if trend_percentage > 20:
            ...
            description = f"Strong upward momentum with {trend_percentage:.1f}% increase in activity"
        elif trend_percentage < -20:
            classification = "declining"
            description = f"Concerning decline with {abs(trend_percentage):.1f}% decrease in activity"
        else:
    ...
    def _calculate_project_health_indicators(self, project_commits: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        ticket_coverage = (commits_with_tickets / len(project_commits)) * 100 if project_commits else 0
        ...
        indicators = {
            ...
            "contributor_diversity": {
                ...
                "status": "excellent" if len(contributors) >= 4 else "good" if len(contributors) >= 2 else "concerning"
            },
            "pr_velocity": {
            ...
            "ticket_coverage": {
                ...
                "status": "excellent" if ticket_coverage >= 80 else "good" if ticket_coverage >= 60 else "needs_improvement"
            }
        }
    ...
    def _detect_technical_debt_signals(self, project_commits: list[dict[str, Any]], context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for commit in project_commits:
            lines_changed = (commit.get('filtered_insertions', commit.get('insertions', 0)) + 
    ...
    def _assess_project_risks(self, project_commits: list[dict[str, Any]], context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        recent_commits = [c for c in project_commits 
                         if (datetime.now(timezone.utc) - c['timestamp']).days <= 14]
    ...
    def _generate_project_recommendations(
        ...
        if high_severity_debt:
            recommendations.append({
                ...
                "action": high_severity_debt[0].get('recommendation', 'Improve code quality practices'),
        ...
        if high_severity_risks:
            recommendations.append({
                ...
                "action": high_severity_risks[0].get('mitigation', 'Address identified risk factors'),
    ...
    def _generate_project_narrative(
        ...
        if high_risks:
            narrative_parts.append(f"Critical attention needed for {high_risks[0]['type'].replace('_', ' ')} risk.")
    ...
    def _analyze_contribution_patterns(self, dev_commits: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        for commit in dev_commits:
            lines = (commit.get('filtered_insertions', commit.get('insertions', 0)) + 
                    commit.get('filtered_deletions', commit.get('deletions', 0)))
            commit_sizes.append(lines)
        
        avg_commit_size = statistics.mean(commit_sizes) if commit_sizes else 0
        size_consistency = 100 - (statistics.pstdev(commit_sizes) / max(avg_commit_size, 1) * 100) if len(commit_sizes) > 1 else 50
        
        # Pattern classification
        total_commits = len(dev_commits)
        
        if total_commits >= self.thresholds['high_productivity_commits'] and consistency_rate >= 0.7:
    ...
    def _get_pattern_description(self, pattern: str, consistency_rate: float, total_commits: int) -> str:
    ...
    def _identify_expertise_domains(self, dev_commits: list[dict[str, Any]], context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for project, commit_count in project_contributions.items():
            ...
            domains.append({
                ...
                "confidence": min(0.9, commit_count / 20)  # Higher confidence with more commits
            })
    ...
    def _analyze_growth_trajectory(self, dev_commits: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(quarter_complexities) >= 2 and len(quarter_projects) >= 2:
            complexity_trend = (quarter_complexities[-1] - quarter_complexities[0]) / max(quarter_complexities[0], 1)
        ...
        return {
            ...
            "confidence": confidence,
            "description": description,
            "complexity_trend": round(complexity_trend * 100, 1) if 'complexity_trend' in locals() else 0,
    ...
    def _detect_burnout_indicators(self, dev_commits: list[dict[str, Any]], context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        weekend_commits = sum(1 for c in dev_commits 
    ...
    def _generate_developer_narrative(
        ...
        if expertise_domains and len(expertise_domains) > 0:
            ...
            if len(expertise_domains) == 1:
                narrative_parts.append(f"Primary expertise in {primary_domain['domain']} with {primary_domain['expertise_level']} level proficiency.")
            else:
                narrative_parts.append(f"Multi-domain contributor with {primary_domain['expertise_level']} expertise in {primary_domain['domain']} and experience across {len(expertise_domains)} areas.")
        
        # Growth trajectory
        trajectory = growth_trajectory.get('trajectory', 'stable')
        trajectory_desc = growth_trajectory.get('description', '')
        if trajectory == 'growing':
            narrative_parts.append(f"Shows positive growth trajectory with {trajectory_desc.lower()}.")
    ...
    def _assess_git_pm_correlation(
        ...
        if total_correlations > 0:
            quality_score = (high_confidence * 1.0 + medium_confidence * 0.6) / total_correlations * 100
        else:
            quality_score = 0
    ...
    def _identify_automation_opportunities(self, commits: list[dict[str, Any]], context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        fix_commits = sum(1 for c in commits 
                         if any(keyword in c.get('message', '').lower() 
        ...
        deploy_commits = sum(1 for c in commits 
                            if any(keyword in c.get('message', '').lower() 
    ...
    def _calculate_compliance_metrics(
        ...
        descriptive_messages = sum(1 for c in commits 
                                  if len(c.get('message', '').split()) >= 3)  # At least 3 words
        message_quality = (descriptive_messages / total_commits) * 100 if total_commits else 0
        
        # Size compliance (reasonable commit sizes)
        appropriate_size_commits = sum(1 for c in commits 
                                     if 10 <= (c.get('filtered_insertions', c.get('insertions', 0)) + 
                                              c.get('filtered_deletions', c.get('deletions', 0))) <= 300)
        ...
        return {
            ...
            "message_quality": {
                ...
                "status": "excellent" if message_quality >= 80 else "good" if message_quality >= 60 else "needs_improvement"
            },
            "commit_size_compliance": {
                "score": round(size_compliance, 1),
                "status": "excellent" if size_compliance >= 80 else "good" if size_compliance >= 60 else "needs_improvement"
            },
            "pr_approval_rate": {
                "score": pr_approval_rate,
    ...
    def _analyze_team_collaboration_patterns(self, commits: list[dict[str, Any]], context: dict[str, Any]) -> dict[str, Any]:
        ...
        avg_contributors_per_project = statistics.mean([len(contributors) for contributors in project_contributors.values()]) if project_contributors else 0
    ...
    def _generate_workflow_narrative(
        ...
        if compliance_score >= 80:
            ...
            narrative_parts.append("Development processes are generally well-followed with room for improvement.")
        else:
            narrative_parts.append("Development processes need attention to improve compliance and effectiveness.")
        
        # Bottleneck highlights
        high_severity_bottlenecks = [b for b in bottlenecks if b.get('severity') == 'high']
        if high_severity_bottlenecks:
            narrative_parts.append(f"Critical bottleneck identified: {high_severity_bottlenecks[0]['type'].replace('_', ' ')}.")
        elif bottlenecks:
            narrative_parts.append(f"Some process inefficiencies detected, particularly in {bottlenecks[0]['type'].replace('_', ' ')}.")
    ...
    def _generate_cross_insights(
        ...
        declining_projects = sum(1 for p in project_analysis.values() 
                               if p.get('momentum', {}).get('classification') == 'declining')
        declining_developers = sum(1 for d in developer_analysis.values() 
