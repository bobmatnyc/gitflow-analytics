...
class FeatureExtractor:
    ...
    def __init__(self):
        ...
        self.keyword_categories = {
            'feature_keywords': [
                ...
                'feature', 'new', 'functionality', 'capability', 'enhancement'
            ],
            'bugfix_keywords': [
                'fix', 'bug', 'issue', 'resolve', 'correct', 'repair', 'patch',
                'error', 'problem', 'defect', 'broken', 'wrong', 'crash'
            ],
            'refactor_keywords': [
                'refactor', 'restructure', 'cleanup', 'optimize', 'improve',
                'simplify', 'reorganize', 'consolidate', 'streamline'
            ],
            'docs_keywords': [
                'doc', 'docs', 'documentation', 'readme', 'comment', 'explain',
                'guide', 'tutorial', 'example', 'specification', 'manual'
            ],
            'test_keywords': [
            ...
            'config_keywords': [
                'config', 'configuration', 'setting', 'environment', 'setup',
                'property', 'parameter', 'option', 'flag', 'variable'
            ],
            'security_keywords': [
                'security', 'secure', 'auth', 'authentication', 'authorization',
                'permission', 'vulnerability', 'exploit', 'sanitize', 'validate'
            ],
            'performance_keywords': [
                'performance', 'optimize', 'fast', 'slow', 'cache', 'memory',
                'cpu', 'speed', 'efficient', 'bottleneck', 'profile'
            ],
            'ui_keywords': [
                'ui', 'interface', 'frontend', 'design', 'layout', 'style',
                'component', 'widget', 'view', 'screen', 'page'
            ],
            'api_keywords': [
                'api', 'endpoint', 'service', 'backend', 'server', 'client',
                'request', 'response', 'http', 'rest', 'graphql'
            ],
            'database_keywords': [
                'database', 'db', 'sql', 'query', 'table', 'schema',
                'migration', 'model', 'data', 'repository'
            ],
            'deployment_keywords': [
                'deploy', 'deployment', 'release', 'build', 'ci', 'cd',
                'docker', 'kubernetes', 'infrastructure', 'production'
            ],
            'dependency_keywords': [
            ...
            'maintenance_keywords': [
                'maintenance', 'cleanup', 'housekeeping', 'chore', 'routine',
                'update', 'bump', 'remove', 'delete', 'deprecated'
            ],
            'hotfix_keywords': [
                'hotfix', 'urgent', 'critical', 'emergency', 'immediate',
                'asap', 'production', 'live', 'quick', 'temporary'
            ],
            'merge_keywords': [
                'merge', 'cherry-pick', 'rebase', 'conflict', 'branch',
                'pull', 'request', 'pr', 'integration', 'combine'
            ],
            'revert_keywords': [
                'revert', 'rollback', 'undo', 'back', 'restore', 'reset',
                'previous', 'original', 'cancel', 'abort'
            ],
            'wip_keywords': [
                'wip', 'progress', 'partial', 'incomplete', 'draft',
                'temporary', 'placeholder', 'todo', 'fixme', 'hack'
            ],
            'breaking_keywords': [
                'breaking', 'break', 'incompatible', 'major', 'change',
                'migration', 'upgrade', 'deprecated', 'removed', 'api'
            ],
            'experimental_keywords': [
                'experimental', 'prototype', 'poc', 'spike', 'trial',
                'test', 'experiment', 'explore', 'research', 'investigate'
            ]
        }
    ...
    def _compile_keyword_patterns(self) -> None:
        ...
        for category, keywords in self.keyword_categories.items():
            ...
            self.compiled_keyword_patterns[category] = re.compile(
                combined_pattern, re.IGNORECASE
            )
    
    def extract_features(self, commit_data: dict[str, Any], 
                        author_stats: Optional[dict[str, Any]] = None) -> np.ndarray:
    ...
    def _extract_author_features(self, commit_data: dict[str, Any],
                                author_stats: Optional[dict[str, Any]] = None) -> np.ndarray:
        ...
        is_automated = any(indicator in author_name.lower() or indicator in author_email.lower() 
    ...
    def get_feature_names(self) -> list[str]:
        ...
        # File features (20)
        file_feature_names = [
            'lang_1st', 'lang_2nd', 'lang_3rd', 'lang_4th', 'lang_5th',
            'activity_1st', 'activity_2nd', 'activity_3rd', 'activity_4th', 'activity_5th',
            'lang_diversity', 'activity_diversity', 'generated_ratio',
            'is_multilingual', 'is_cross_functional',
            'ext_py', 'ext_js', 'ext_java', 'ext_go', 'ext_sql'
        ]
        names.extend(file_feature_names)
        
        # Statistics features (15)
        stats_feature_names = [
            'files_changed', 'insertions', 'deletions', 'total_lines', 'insert_delete_ratio',
            'message_length', 'word_count', 'line_count', 'avg_lines_per_file',
            'has_conventional_format', 'has_ticket_ref', 'is_merge', 'has_code_in_msg',
            'message_complexity', 'is_large_commit'
        ]
        ...
        temporal_feature_names = [
            ...
            'is_weekend', 'is_business_hours', 'is_late_night', 'quarter'
        ]
        names.extend(temporal_feature_names)
        
        # Author features (5)
        author_feature_names = [
            'author_name_length', 'has_corporate_email', 'is_automated',
            'author_experience', 'commit_size_vs_typical'
        ]
        names.extend(author_feature_names)
        
        return names
    
    def extract_batch_features(self, commit_batch: list[dict[str, Any]], 
                              author_stats_batch: Optional[list[dict[str, Any]]] = None) -> np.ndarray:
