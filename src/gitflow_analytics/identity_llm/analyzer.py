...
class LLMIdentityAnalyzer:
    ...
    def analyze_identities(self, 
        ...
        unresolved = [
            identity for identity in identities.values()
    ...
    def _are_likely_same_developer(self, id1: DeveloperAlias, id2: DeveloperAlias) -> bool:
        ...
        if match1 or match2:
            ...
            if github_username1:
                ...
                if (github_username1 in other_name or other_name in github_username1 or
                    github_username1 in other_local or other_local in github_username1):
                    return True
            ...
            if github_username2:
                ...
                if (github_username2 in other_name or other_name in github_username2 or
                    github_username2 in other_local or other_local in github_username2):
                    return True
        ...
        if (clean_local1 in name2_clean or name2_clean in clean_local1 or
            clean_local2 in name1_clean or name1_clean in clean_local2):
            return True
        ...
        if name_similarity > 0.8 and (domain1 == domain2 or 
                                     'github.com' in domain1 or 
                                     'github.com' in domain2):
            return True
    ...
    def _finalize_heuristic_clusters(self, 
        ...
        for email_set in pre_clusters:
            ...
            cluster_identities = [
                identity for identity in identities.values() 
                if identity.email in email_set
            ]
        ...
        try:
            ...
            for email_set in pre_clusters:
                ...
                response = client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an expert at analyzing developer identities and determining if different email/name combinations belong to the same person."},
                ...
                # Parse LLM response
                cluster = self._parse_llm_response(response.choices[0].message.content, 
                                                 cluster_identities,
                                                 identities)
                if cluster:
                    clusters.append(cluster)
        ...
        for group in name_groups.values():
            ...
            try:
                response = client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an expert at analyzing developer identities."},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.3,
                    max_tokens=500
