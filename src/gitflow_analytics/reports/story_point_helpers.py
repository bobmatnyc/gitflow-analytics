...
class StoryPointHelpersMixin:
    ...
    def _calculate_accuracy_metrics(
        self, estimation_pairs: list[tuple[int, int, str]]
    ) -> dict[str, Any]:
        ...
        accuracy_percentage = (
            (accurate_estimates / len(estimation_pairs)) * 100 if estimation_pairs else 0
        )
        ...
        return {
            ...
            "correlation_coefficient": float(stats.pearsonr(estimated_values, actual_values)[0])
            if len(estimation_pairs) > 1
            else 0,
        }

    def _analyze_developer_estimation_accuracy(
        self, estimation_pairs: list[tuple[int, int, str]]
        ...
        for developer, pairs in developer_pairs.items():
            if len(pairs) >= 2:  # Need multiple estimates for meaningful analysis
                ...
                mae = float(
                    np.mean([abs(est - act) for est, act in zip(estimated_values, actual_values)])
                )
                ...
                developer_accuracy[self._anonymize_value(developer, "name")] = {
                    ...
                    "consistency": 1.0 - float(np.std(relative_errors) / 100)
                    if relative_errors
                    else 0,
                }
    ...
    def _analyze_size_based_accuracy(
        self, estimation_pairs: list[tuple[int, int, str]]
    ) -> dict[str, dict[str, Any]]:
        ...
        for size_name, (min_sp, max_sp) in size_ranges.items():
            size_pairs = [(est, act) for est, act, _ in estimation_pairs if min_sp <= est <= max_sp]
    ...
    def _aggregate_weekly_velocity(
        ...
        for commit in commits:
            ...
            weekly_velocity[week_key]["story_points_completed"] += (
    ...
    def _calculate_velocity_trends(
        self, weekly_velocity: dict[str, dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        for dev_id, metrics in developer_metrics.items():
            if metrics["total_commits"] > 0:
                ...
                developer_velocity[self._anonymize_value(dev_id, "name")] = {
                    ...
                    "story_points_per_commit": metrics["total_story_points"]
                    / metrics["total_commits"],
                }

        return developer_velocity

    def _calculate_velocity_predictability(
        self, weekly_velocity: dict[str, dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def _analyze_team_capacity(
        self,
        weekly_velocity: dict[str, dict[str, Any]],
        developer_velocity: dict[str, dict[str, Any]],
    ) -> dict[str, Any]:
        ...
        gini = (n + 1 - 2 * sum((n + 1 - i) * x for i, x in enumerate(sorted_contributions, 1))) / (
            n * sum(sorted_contributions)
        )
        ...
        if gini > 0.4:  # High inequality
            recommendations.append(
                "Consider redistributing workload - significant imbalance detected"
            )

        # Check for low contributors
        low_contributors = [
            dev
            for dev, metrics in developer_velocity.items()
            if metrics["avg_weekly_velocity"]
            < np.mean([m["avg_weekly_velocity"] for m in developer_velocity.values()]) * 0.5
        ]

        if low_contributors:
            recommendations.append(
                f"Support developers with low velocity: {', '.join(low_contributors[:3])}"
            )
        ...
        return {
            ...
            "avg_weekly_team_velocity": float(
                np.mean([w["story_points_completed"] for w in weekly_velocity.values()])
            ),
            ...
            "top_contributors": sorted(
                [
                    (dev, metrics["total_story_points"])
                    for dev, metrics in developer_velocity.items()
                ],
                key=lambda x: x[1],
                reverse=True,
    ...
    def _empty_week_correlations(self) -> dict[str, Any]:
        ...
        return {
            "correlations": {
                k: 0.0 for k in ["sp_commits", "sp_lines", "sp_files", "sp_prs", "sp_complexity"]
            },
            "sample_size": 0,
            "total_story_points": 0,
            "total_commits": 0,
    ...
    def _generate_accuracy_recommendations(
        ...
        developer_accuracy: dict[str, dict[str, Any]],
        size_accuracy: dict[str, dict[str, Any]],
    ) -> list[dict[str, str]]:
        ...
        low_accuracy_devs = [
            dev
            for dev, stats in developer_accuracy.items()
