        ...
        with open(output_path, "w", newline="", encoding="utf-8") as f:
            ...
            if confidence_scores:
                ...
                writer.writerow(
                    [
                        "Very High (≥0.9)",
                        f"{very_high} ({(very_high/len(confidence_scores))*100:.1f}%)",
                    ]
                )
                writer.writerow(
                    ["High (0.8-0.9)", f"{high} ({(high/len(confidence_scores))*100:.1f}%)"]
                )
                writer.writerow(
                    ["Medium (0.6-0.8)", f"{medium} ({(medium/len(confidence_scores))*100:.1f}%)"]
                )
                writer.writerow(
                    ["Low (0.4-0.6)", f"{low} ({(low/len(confidence_scores))*100:.1f}%)"]
                )
                writer.writerow(
                    [
                        "Very Low (<0.4)",
                        f"{very_low} ({(very_low/len(confidence_scores))*100:.1f}%)",
                    ]
                )
                writer.writerow([])
