...
def example_basic_usage():
    ...
    output = create_report(
        ReportType.DEVELOPER_STATS, ReportFormat.CSV, report_data, "developer_stats.csv"
    )
...
def example_custom_generator():
    ...
    class CustomHTMLGenerator(BaseReportGenerator):
        ...
        def generate(self, data: ReportData, output_path: Path = None) -> ReportOutput:
            ...
            if output_path:
                ...
                return ReportOutput(
                    success=True, file_path=output_path, format="html", size_bytes=len(html_content)
                )
            else:
                return ReportOutput(
                    success=True, content=html_content, format="html", size_bytes=len(html_content)
                )
        ...
        def _generate_html(self, data: ReportData) -> str:
            ...
            for dev in sorted(
                data.developer_stats, key=lambda d: d.get("total_commits", 0), reverse=True
            )[:10]:
                html += f"""
                    <tr>
    ...
    factory = ReportFactory()
    factory.register_generator(ReportType.CUSTOM, ReportFormat.HTML, CustomHTMLGenerator)
