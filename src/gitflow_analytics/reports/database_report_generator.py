...
class DatabaseReportGenerator:
    ...
    def generate_qualitative_report(
        ...
        classification_summary = self.storage.get_classification_summary(start_date, end_date)
    ...
    def _build_executive_summary(
        ...
        top_category = (
            max(category_totals, key=category_totals.get) if category_totals else "feature"
        )
        ...
        top_contributor_pct = (
            (top_contributor_commits / total_commits * 100) if total_commits > 0 else 0
        )
    ...
    def _build_team_analysis(self, developer_summary: Dict[str, Dict[str, int]]) -> List[str]:
        ...
        sorted_devs = sorted(developer_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    ...
    def _build_project_analysis(self, project_summary: Dict[str, Dict[str, int]]) -> List[str]:
        ...
        sorted_projects = sorted(project_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    ...
    def _build_recommendations(
        ...
        if recommendations:
            ...
            lines.append(
                "No specific recommendations at this time. Overall development patterns look healthy."
            )
            lines.append("")
