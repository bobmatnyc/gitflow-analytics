...
class PullRequestData:
    ...
    Fields are populated from the GitHub integration layer.  Enhanced review
    fields (approvals_count, change_requests_count, reviewers, approved_by,
    time_to_first_review_hours, revision_count) are only available when
    ``fetch_pr_reviews`` is enabled in the GitHub integration config.
    """
    ...
    # Comment metrics
    # comments_count: general PR timeline comments (issue-style)
    comments_count: int = 0
    # review_comments_count: inline code review comments
    ...
    # Review decision metrics (populated when fetch_pr_reviews is enabled)
    approvals_count: int = 0
    change_requests_count: int = 0

    # Revision estimate (number of commit-push cycles after initial open)
    ...
    def was_approved(self) -> bool:
        """Return True if the PR received at least one approval."""
        return self.approvals_count > 0 or len(self.approved_by) > 0

    def had_change_requests(self) -> bool:
        """Return True if the PR received at least one change request."""
        return self.change_requests_count > 0
...
class ProjectMetrics:
    ...
    # Developer breakdown
    developer_contributions: dict[str, float] = field(
