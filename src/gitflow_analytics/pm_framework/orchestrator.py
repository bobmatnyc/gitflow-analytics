...
class PMFrameworkOrchestrator:
    ...
    def _initialize_platforms(self) -> None:
        ...
        for platform_name, platform_config in platforms_config.items():
            ...
            try:
                ...
                logger.info(f"✅ {platform_name} initialized: {connection_info.get('status', 'unknown')}")
            ...
            try:
                ...
                for project in projects:
                    try:
                        ...
                        logger.info(f"🎫 Found {len(issues)} issues in {platform_name}/{project.key}")
                        
                    except Exception as e:
                        logger.error(f"⚠️ Failed to get issues from {platform_name}/{project.key}: {e}")
    ...
    def correlate_issues_with_commits(self, issues: dict[str, list[UnifiedIssue]], 
        ...
        logger.info(f"Correlating {sum(len(i) for i in issues.values())} issues with {len(commits)} commits")
        ...
        if self.fuzzy_matching_enabled:
            fuzzy_correlations = self._correlate_by_fuzzy_matching(issue_lookup, commits, direct_correlations)
    ...
    def _correlate_by_ticket_references(self, issue_lookup: dict[str, UnifiedIssue], 
                                      commits: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _correlate_by_fuzzy_matching(self, issue_lookup: dict[str, UnifiedIssue],
                                   commits: list[dict[str, Any]],
                                   existing_correlations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def calculate_enhanced_metrics(self, commits: list[dict[str, Any]], 
        ...
        metrics['story_point_analysis'] = {
            ...
            'story_point_coverage_pct': (issues_with_story_points / total_issues * 100) if total_issues > 0 else 0,
        ...
        for platform, issues in pm_issues.items():
            linked_issues = [c['issue_key'] for c in correlations if c.get('issue_platform') == platform]
            ...
            platform_coverage[platform] = {
                ...
                'correlation_rate': len(linked_issues) / len(correlations) * 100 if correlations else 0
            }
        ...
        if correlations:
            ...
            metrics['correlation_quality'] = {
                ...
                'high_confidence_correlations': sum(1 for score in confidence_scores if score >= self.confidence_threshold),
