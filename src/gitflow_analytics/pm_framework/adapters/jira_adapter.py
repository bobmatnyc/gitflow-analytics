...
class JiraTicketCache:
    ...
    def store_ticket(self, ticket_key: str, ticket_data: dict[str, Any]) -> None:
        ...
        assignee = ticket_data.get('assignee', {}).get('display_name') if ticket_data.get('assignee') else None
        reporter = ticket_data.get('reporter', {}).get('display_name') if ticket_data.get('reporter') else None
        ...
        with sqlite3.connect(self.cache_path) as conn:
            ...
            """, (
                ticket_key, project_key, json.dumps(ticket_data), story_points,
                status, issue_type, assignee, reporter, created_at, updated_at,
    ...
    def get_project_tickets(self, project_key: str, include_expired: bool = False) -> list[dict[str, Any]]:
    ...
    def print_cache_summary(self) -> None:
        ...
        print(f"   • Total Tickets: {stats['total_tickets']:,} ({stats['fresh_tickets']:,} fresh, {stats['expired_tickets']:,} expired)")
        ...
        if stats['total_requests'] > 0:
            print(f"\n⚡ Session Performance:")
            print(f"   • Hit Rate: {stats['hit_rate_percent']:.1f}% ({stats['cache_hits']:,}/{stats['total_requests']:,})")
        ...
        if stats['hit_rate_percent'] > 80:
            ...
            print(f"   ⚠️  Consider adjusting TTL or clearing stale entries")
...
class JIRAAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        # Debug output
        logger.info(f"JIRA adapter init: base_url={self.base_url}, username={self.username}, has_token={bool(self.api_token and self.api_token != 'dummy-token')}")
        print(f"   🔍 JIRA adapter received: username={self.username}, has_token={bool(self.api_token and self.api_token != 'dummy-token')}, base_url={self.base_url}")
