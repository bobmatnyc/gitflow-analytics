...
class SchemaVersion(Base):
    ...
    last_processed_date = Column(DateTime, nullable=True)  # Last date we processed data with this schema
...
class SchemaVersionManager:
    ...
    CURRENT_SCHEMAS = {
        'qualitative': {
            'version': '2.0',
            'fields': [
                'change_type', 'change_type_confidence',
                'business_domain', 'domain_confidence', 
                'risk_level', 'risk_factors',
                'intent_signals', 'collaboration_patterns',
                'technical_context', 'processing_method',
                'processing_time_ms', 'confidence_score'
            ],
            'config_fields': [
                'nlp_config', 'llm_config', 'cache_config',
                'confidence_threshold', 'max_llm_fallback_pct'
            ]
        },
        'identity': {
            'version': '1.3',
            'fields': [
                ...
                'manual_mappings', 'similarity_threshold',
                'auto_analysis', 'display_names', 'preferred_name_field'
            ]
        },
        'core': {
            'version': '1.0', 
            'fields': [
                'story_points', 'ticket_references',
                'files_changed', 'insertions', 'deletions',
                'complexity_delta', 'branch_mapping_rules'
            ]
        },
        'github': {
            'version': '1.0',
            'fields': [
                'pr_data', 'pr_metrics', 'issue_data',
                'rate_limit_retries', 'backoff_factor',
        ...
        'jira': {
            'version': '1.0',
            'fields': [
                'story_point_fields', 'project_keys',
                'base_url', 'issue_data'
            ]
        }
    }
    ...
    def update_schema_version(self, component: str, config: Optional[Dict[str, Any]] = None, 
    ...
    def should_process_date(self, component: str, date: datetime, 
    ...
    def mark_date_processed(self, component: str, date: datetime, 
        ...
        with self.session_factory() as session:
            ...
            if stored_version:
                ...
                if not stored_version.last_processed_date or date > stored_version.last_processed_date:
