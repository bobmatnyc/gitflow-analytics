...
from ..models.database import DailyMetrics, Database, WeeklyTrends
...
class DailyMetricsStorage:
    ...
    def get_classification_summary(
        ...
        with self.get_session() as session:
            ...
            ).group_by(DailyMetrics.developer_name).all()
            
            # Classification totals by project
            proj_query = session.query(
                DailyMetrics.project_key,
                func.sum(DailyMetrics.feature_commits).label('features'),
                func.sum(DailyMetrics.bug_fix_commits).label('bug_fixes'),
                func.sum(DailyMetrics.refactor_commits).label('refactors'),
            ...
            ).group_by(DailyMetrics.project_key).all()
    ...
    def _aggregate_commits_by_day(
        ...
        for commit in commits:
            ...
            dev_identity = developer_identities.get(author_email, {
            ...
            metrics['developer_name'] = dev_identity.get('name', commit.get('author_name', 'Unknown'))
    ...
    def _calculate_developer_project_trend(
        ...
        return {
            'total_commits_change': calculate_change(
                current_week['total_commits'], 
                previous_week['total_commits']
            ),
            'feature_commits_change': calculate_change(
                current_week['feature_commits'], 
                previous_week['feature_commits']
            ),
            'bug_fix_commits_change': calculate_change(
                current_week['bug_fix_commits'], 
                previous_week['bug_fix_commits']
            ),
            'refactor_commits_change': calculate_change(
                current_week['refactor_commits'], 
                previous_week['refactor_commits']
            ),
