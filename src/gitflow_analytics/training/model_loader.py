...
class TrainingModelLoader:
    ...
    def get_best_model(self) -> Optional[dict[str, Any]]:
        ...
        with self.db.get_session() as session:
            best_model = session.query(ClassificationModel).filter_by(
                active=True
            ).order_by(
                ClassificationModel.validation_accuracy.desc()
            ).first()
    ...
    def load_model(self, model_id: Optional[str] = None) -> tuple[Any, dict[str, Any]]:
        ...
        if not model_info:
            raise ValueError(f"No model found with ID: {model_id}" if model_id else "No trained models available")
        ...
        try:
            ...
            logger.info(f"Loaded model {model_info['model_id']} v{model_info['version']} ({model_info['accuracy']:.3f} accuracy)")
    ...
    def _get_model_by_id(self, model_id: str) -> Optional[dict[str, Any]]:
        ...
        """
        with self.db.get_session() as session:
            model = session.query(ClassificationModel).filter_by(
                model_id=model_id,
                active=True
            ).first()
    ...
    def list_available_models(self) -> list[dict[str, Any]]:
        ...
        with self.db.get_session() as session:
            db_models = session.query(ClassificationModel).filter_by(
                active=True
            ).order_by(
                ClassificationModel.validation_accuracy.desc()
            ).all()
    ...
    def predict_commit_category(self, 
                              message: str, 
        ...
        try:
            ...
            result = {
                ...
                'alternatives': self._get_alternative_predictions(prediction_scores[0], model.classes_),
