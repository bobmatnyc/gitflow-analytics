...
class CommitClassificationTrainer:
    ...
    def __init__(self, config: Config, cache: GitAnalysisCache, 
                 orchestrator: Any, training_config: Optional[dict[str, Any]] = None):
        ...
        self.classifier = CommitClassifier(
            config=config.analysis.commit_classification.__dict__ if hasattr(config.analysis, 'commit_classification') else {},
            cache_dir=cache.cache_dir
        )
        
        logger.info(f"Initialized training pipeline with cache at {cache.cache_dir}")
    
    def train(self, repositories: list[Any], since: datetime, 
    ...
    def _extract_labeled_commits(self, repositories: list[Any], 
        ...
        analyzer = GitAnalyzer(
            ...
            allowed_ticket_platforms=getattr(self.config.analysis, 'allowed_ticket_platforms', None)
        )
    ...
    def get_training_history(self) -> list[dict[str, Any]]:
        ...
        """
        with self.Session() as db_session:
            sessions = db_session.query(TrainingSession).order_by(
