...
class ActivityVerifier:
            ...
            for repo_info in pbar:
                ...
                try:
                    ...
                    self._update_daily_matrix(
                        results["daily_matrix"],
                        repo_info["name"],
                        project_data["daily_commits"]
                    )
            ...
            if isinstance(self.config.repositories, list):
                for repo_config in self.config.repositories:
                    ...
                    if hasattr(repo_config, 'name'):
                        ...
                        github_repo = repo_config.github_repo if hasattr(repo_config, 'github_repo') else None
                    # Handle dict format
                    else:
        ...
        try:
            ...
            for branch in repo.get_branches():
                try:
                    ...
                    if not result["last_activity"] or commit_date > datetime.fromisoformat(result["last_activity"]):
                        result["last_activity"] = commit_date.isoformat()
                except Exception as e:
                    logger.debug(f"Error processing branch {branch.name}: {e}")

            result["branches"] = sorted(branches_data, key=lambda x: x["last_activity"], reverse=True)
    ...
    def _verify_local_activity(self, repo_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            all_refs = list(repo.heads) + [ref for remote in repo.remotes for ref in remote.refs if not ref.name.endswith("/HEAD")]
    ...
    def format_report(self, results: Dict[str, Any]) -> str:
        ...
        lines.append("=" * 80)
        lines.append(f"Period: {results['period']['start'][:10]} to {results['period']['end'][:10]}")
        ...
        if results['summary']['inactive_projects']:
            lines.append(f"  Projects with no activity: {', '.join(results['summary']['inactive_projects'])}")
        lines.append("")
        ...
        projects = sorted(set(
            project for date_data in results['daily_matrix'].values()
        ...
        for date_str in sorted(results['daily_matrix'].keys()):
            total_commits = sum(
                results['daily_matrix'][date_str].get(p, 0)
                for p in projects
            ) if projects else 0
...
def verify_activity_command(
    ...
    zero_days = len([
        d for d in results['daily_matrix'].keys()
        if sum(results['daily_matrix'][d].values()) == 0
    ])
    if zero_days > 0:
